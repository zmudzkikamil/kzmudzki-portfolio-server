...
export async function seedExperience(dataSource: DataSource): Promise<void> {
  ...
  const count = await experienceRepository.count();
  if (count > 0) {
    console.log('✓ Experience data already exists, skipping...');
    return;
  }
  ...
  // Insert Positions for Freelancer
  await positionRepository.save([
    {
      title: 'Web Developer',
      period: '2021 - 2022',
